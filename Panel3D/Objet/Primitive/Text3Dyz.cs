...
class Text3Dyz : Object3D
{
    ...
    public Text3Dyz(Panel3D _Panel3D, Vector3 _position, Vector3 _direction, float _scale, String _text)
        : base(_Panel3D)
    {
        position = _position;
    ...
    {
        float angle2 = (float)Math.Acos(productSca(new Vector3(0f, -1f, 0f), direction));
        Vector3 sens = productVect(new Vector3(-1f, 0f, 0f), direction);
        if (sens.Z < 0) angle2 = -angle2;

        transform = Matrix.Multiply(Matrix.Scaling(scale, scale, scale), Matrix.RotationAxis(new Vector3(1f, 0f, 0f), (float)Math.Acos(0)));
        transform = Matrix.Multiply(transform, Matrix.RotationAxis(new Vector3(0f, 0f, -1f), (float)Math.Acos(0)));
        transform = Matrix.Multiply(transform, Matrix.RotationAxis(new Vector3(1f, 0f, 0f), angle2));
